                ...
                <tbody>
                    {filteredList.map((g) => (
                        <tr key={g.girviNumber} className="border-b hover:bg-gray-50 transition duration-150">
                            ...
                            <td className="p-3">
                                <span
                                    className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${g.status === "Active"
...
export default function GirviPage() {
    ...
    const handleFormSubmit = (formData) => {
        ...
        const updatedList = [...girviListState, newGirvi];
        setGirviListState(updatedList);
        localStorage.setItem("girvi_records", JSON.stringify(updatedList));
    ...
    const handleUpdate = useCallback((girviNumber, updates) => {
        setGirviListState(prevList => {
            const newList = prevList.map(g =>
                g.girviNumber === girviNumber ? { ...g, ...updates } : g
            );
            localStorage.setItem("girvi_records", JSON.stringify(newList));
            return newList;
        });
    ...
    return (
        <div className="space-y-6 p-4 sm:px-4 md:px-6 lg:px-4 pb-24">
            ...
            <div className="flex justify-between flex-col md:flex-row gap-4 mb-6">
                ...
                <button
                    onClick={(openModal) => setShowForm(true)}
